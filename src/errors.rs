...
pub enum OffsetDumperError {
    ...
    PdbFileNotFoundError,
}

impl Display for OffsetDumperError {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        match self {
            OffsetDumperError::Radare2NotFoundError => write!(
                f,
            ...
            OffsetDumperError::NtoskrnlNotValidError => {
                write!(f, "The configured ntoskrnl.exe does not exist or is not a file.")
            }
            OffsetDumperError::NtoskrnlVersionNotFoundError => write!(
                f,
                "Radare2 could not find ntoskrnl.exe file version. Please double-check your ntoskrnl file."
            ),
            OffsetDumperError::NtoskrnlDownloadingPdbError => write!(
                f,
                "Radare2 could not download PDB files for this file version. Please double-check your ntoskrnl file."
            ),
            OffsetDumperError::NtoskrnlDumpingOffsetsError => {
                write!(f, "Radare2 got an error while dumping ntoskrnl.exe. Sorry!")
            }
            OffsetDumperError::PdbFileNotFoundError => {
                write!(f, "The specified PDB file does not exist.")
            }
        }
    }
}